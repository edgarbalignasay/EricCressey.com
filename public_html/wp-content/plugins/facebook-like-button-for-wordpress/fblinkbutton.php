...
} elseif (get_option('fblike_layout') == 'box count') {
	$button .= '&amp;layout=box_count';
	$wdth = '150';
} else{
...
function fblike_generate_button() {
    ...
    $button = '<div class="fblikebutton_button" style="' . get_option('fblike_style') . '"><span style="float:left; margin-right:5px;"><a href="http://twitter.com/share" class="twitter-share-button" data-count="vertical">Tweet</a><script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script></span>';
$button .= '<iframe src="http://www.facebook.com/plugins/like.php?href=' . fblike_build_options() . '" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; height:62px;"></iframe>';
	$button .= '</div>';
...
function fblike_generate_static_button() {
	if (get_post_status($post->ID) == 'publish') {
		...
		'<div class="fblikebutton_button" style="' . get_option('fblike_style') . '">
			<iframe src="http://www.facebook.com/plugins/like.php?href=' . fblike_build_options() . '" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; height:62px;"></iframe>
		</div>';
	} else {
		return;
	}
            ...
            <tr>
                ...
                <td>
                	<select name="fblike_layout">
                		...
                		<option <?php if (get_option('fblike_layout') == 'count') echo 'selected="selected"'; ?> value="count">Button Count</option>
						<option <?php if (get_option('fblike_layout') == 'box count') echo 'selected="selected"'; ?> value="box count">Box Count</option>
                	</select>
                </td>
            </tr>
